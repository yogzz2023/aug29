...
def main():
    ...
    for group_idx, group in enumerate(measurement_groups):
        ...
        if len(group) > 1:
            # If there are multiple measurements in the same group (same time), find the best match
            print(f"Found {len(group)} measurements with the same timestamp. Calculating optimal assignment...")

            # Convert group measurements to Cartesian coordinates
            measurements_cartesian = [(idx, sph2cart(azm, ele, rng)) for idx, (rng, azm, ele, _) in enumerate(group)]

            # Predict the state
            kalman_filter.predict_step(group[0][3])  # Use the time of the first measurement in the group
            ...
            for track_index, report_index in assignment:
                track_id, track_pos = predicted_states[track_index]
                report_id, report_pos = measurements_cartesian[report_index]
                ...
                Z = np.array([[report_pos[0]], [report_pos[1]], [report_pos[2]]])
                ...
                az.append(az_val)
                el.append(el_val)
                time_list.append(group[0][3])  # Use the timestamp of the group

        else:
            # Process individual measurement
            rng, azm, ele, mt = group[0]
            print(f"Measurement: (az={azm}, el={ele}, r={rng}, t={mt})")
